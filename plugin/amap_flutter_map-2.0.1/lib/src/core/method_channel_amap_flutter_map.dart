...
class MethodChannelAMapFlutterMap implements AMapFlutterPlatform {
  ...
  ///Yioam
  Stream<MarkerEvent> onLwMarkerTap({required int mapId}) {
    return _events(mapId).whereType<MarkerEvent>();
  }
  ...
  Future<dynamic> _handleMethodCall(MethodCall call, int mapId) async {
    switch (call.method) {
      ...
      case 'map#onLongPress':
        ...
        break;
      case 'map#markerTap':
        _mapEventStreamController.add(MarkerEvent(
            mapId, LatLng.fromJson(call.arguments['latLng'])!));
        break;
  ...
  Future<void> addCircle(int mapId, double latitude, double longitude, double radius) {
    return channel(mapId).invokeMethod('map#addCircle', <String, dynamic>{
      'latitude':latitude,
      'longitude':longitude,
      'radius':radius,
    });
  ...
  Future<String?> addMarker(int mapId, Map<String, dynamic> map) {
    return channel(mapId).invokeMethod('map#addMarker', map);
  }

  Future<void> clearMarker(int mapId) {
    return channel(mapId).invokeMethod('map#clearAllMaker');
  }

  Future<void> setMarkerClickToDel(int mapId, bool isClickToDel) {
    return channel(mapId).invokeMethod('map#setMarkerClickToDel',<String, dynamic>{
      'isClickToDel':isClickToDel,
    });
  }

  ///根据屏幕坐标获取经纬度
  Future<String?> fromScreenLocation(int mapId, int x, int y) {
    return channel(mapId).invokeMethod<String?>('map#fromScreenLocation', <String, dynamic>{
      'x':x,
      'y':y,
    });
  }
  ...
  ///marker监听
  Future<String?> setOnMarkerClickListener(int mapId) {
    return channel(mapId).invokeMethod<String?>('map#setOnMarkerClickListener', <String, dynamic>{});
  }

  ///
  Future<String?> drawTrailFromScreenLocation(int mapId, Map<String, dynamic> map) {
    return channel(mapId).invokeMethod<String?>('map#drawTrailFromScreenLocation', map);
  }
