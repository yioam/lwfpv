  ...
  public void onMethodCall(MethodCall call, MethodChannel.Result result) {
    switch (call.method) {
      ...
      case "map#addMarker":
        ...
        break;
      case "map#clearMarker":
        clearTrail();
        break;
      case "map#setMarkerClickToDel":
        isClickToDel = call.argument("isClickToDel");
        break;
      case "map#drawTrailFromScreenLocation":
        if((boolean) call.argument("needClear")){
          clearTrail();
        }
        ...
        ///返回所有的marker经纬度
        break;
      case "map#setOnMarkerClickListener":
          ...
          if(isClickToDel){
            ...
            if(currentIndex != -1){
              ...
              if (currentIndex > 0 && currentIndex < lineLength) {
                ...
                savePolyline.add(currentIndex - 1, googleMap.addPolyline(new PolylineOptions()
                        .addAll(iterable)
              ...
              if(currentIndex < lineLength){
                ...
                for (int i = 0; i < saveMapLatLng.size(); i++) {
                  ...
                  saveMarker.add(googleMap.addMarker(new MarkerOptions().position(saveMapLatLng.get(i))
                          .title("Marker " + (saveMarker.size() + 1))
                          .icon(BitmapDescriptorFactory.fromBitmap(createDrawableFromView(GoogleMapsPlugin.activity, customMarkerView)))));
            ...
            for (int i = 0; i < saveMarker.size(); i++) {
              if(saveMarker.get(i).getId().equals(marker.getId())){
                arguments.put("position", Convert.latLngToJson(saveMapLatLng.get(i)));

                Log.d(TAG, "onMethodCall: ============" + Convert.latLngToJson(saveMapLatLng.get(i)));
...
//            methodChannel.invokeMethod("map#markerTap", arguments);
  ...
  private String drawTrail(double latitude, double longitude, double x, double y, double maxRadius, double calibrate) {
    if (customMarkerView == null) {
      customMarkerView = LayoutInflater.from(GoogleMapsPlugin.activity).inflate(R.layout.custom_markers, null);
    ...
    if (calibrate == 0) {//calibrate ==0 时说明传的经纬度过来
      ...
      Projection gaode_proj = googleMap.getProjection();
      // 点转换为经纬度
    ...
    saveMarker.add(googleMap.addMarker(new MarkerOptions().position(mLatLng)
            .title("Marker " + (saveMarker.size() + 1))
            .icon(BitmapDescriptorFactory.fromBitmap(createDrawableFromView(GoogleMapsPlugin.activity, customMarkerView)))));
    ...
    if (saveMapLatLng.size() >= 2) {
      ...
      savePolyline.add(googleMap.addPolyline(new PolylineOptions()
              .addAll(iterable)
              .width(6).color(Color.BLUE)
              .geodesic(true)));
    }
  ...
  private void addCircle(double latitude, double longitude, double radius){
    if (centerCircle != null) {
      ...
      centerCircle = googleMap.addCircle(new com.google.android.gms.maps.model.CircleOptions()
