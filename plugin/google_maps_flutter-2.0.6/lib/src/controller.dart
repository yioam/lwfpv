...
class GoogleMapController {
  ...
  Future<String?> addMarker(double latitude, double longitude, double curLat, double curLng, double maxRadius,{bool needClear = false}) {
    return GoogleMapsFlutterPlatform.instance.addMarker(<String, dynamic>{
      'latitude': latitude,
      'longitude': longitude,
      'curLat': curLat,
      'curLng': curLng,
  ...
  Future<void> clearMarker(){
    return GoogleMapsFlutterPlatform.instance.clearMarker(mapId: mapId);
  }

  ///设置成点击marker直接删除状态
  Future<String?> setMarkerClickToDel(bool isClickToDel){
    return GoogleMapsFlutterPlatform.instance.setMarkerClickToDel(
        <String, dynamic>{
  ...
  Future<String?> drawTrailFromScreenLocation(
    ...
    return GoogleMapsFlutterPlatform.instance.drawTrailFromScreenLocation(
        <String, dynamic>{
          'latitude': latitude,
          'longitude': longitude,
          'x': x,
  ...
  Future<void> setOnMarkerClickListener(){
    return GoogleMapsFlutterPlatform.instance.setOnMarkerClickListener(mapId: mapId);
  }

  ///Yioam marker回调
  Future<void> onLwMarkerTap(ArgumentCallback<LatLng> lwMarkerTap) {
    GoogleMapsFlutterPlatform.instance.onLwMarkerTap(mapId: mapId).listen(
            (LwMarkerTapEvent e) {///todo longitude的最后以为精度会丢失。。原因未知
              lwMarkerTap(e.position);
            });
